  ...
  try {
    ...
    if (dup) {
      ...
      if (!name || !email || !password || !city || !phone || !role) {
        ...
        return res.status(200).send({
          success: true,
          message: "User inserte",
          userDetails,
        });
      }
    }
    let token = await jwt.sign({ user: user }, "darshna", { expiresIn: "2hr" });
    return res.status(200).send({
      success: true,
      token: token,
    });
...
module.exports = {addUser,loginUser,viewUser, deleteUser, updateUser
};
